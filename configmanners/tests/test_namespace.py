...
import configmanners.config_manager as config_manager
...
class TestCase(unittest.TestCase):
    ...
    def test_Namespace_basics(self):
        ...
        self.assertEqual(namespace.beta.from_string_converter, datetime_from_ISO_string)
    ...
    def test_configuration_with_namespace(self):
        ...
        namespace.b = 17
        config = config_manager.ConfigurationManager([namespace], argv_source=[])
        self.assertTrue(isinstance(config.option_definitions.b, config_manager.Option))
    ...
    def test_namespace_constructor_3(self):
        ...
        config = config_manager.ConfigurationManager([j], argv_source=[])
        self.assertTrue(isinstance(config.option_definitions.a, config_manager.Option))
    ...
    def test_namespace_from_json_with_default_datetime_date(self):
        ...
        https://github.com/twobraids/configmanners/issues/7
        """
        j = (
            u'{"bday": {"default": "1979-12-13", "name": "bday",'
            u' "from_string_converter": "configmanners.datetime_util.date_from_ISO'
            u'_string", "doc": null, "value": "1979-12-13", '
            u'"short_form": null}}'
        )
        config = config_manager.ConfigurationManager(
            [j], [], use_auto_help=False, use_admin_controls=True, argv_source=[]
        )
        ...
        self.assertEqual(config.option_definitions.bday.default, option.default)
    ...
    def test_walk_expanding_class_options(self):
        ...
        c_contents = [
            (qkey, c.option_definitions[qkey])
            for qkey in c.option_definitions.keys_breadth_first(include_dicts=True)
        ]
        c_contents.sort()
        e.sort()
