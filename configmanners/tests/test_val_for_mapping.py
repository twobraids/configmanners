...
from configmanners.value_sources.for_mapping import ValueSource
...
class TestCase(unittest.TestCase):
    ...
    def test_mapping(self):
        ...
        m = {"a": "1", "b": 2, "always_ignore_mismatches": False}
        vs = ValueSource(m)
        self.assertFalse(vs.always_ignore_mismatches)
        self.assertTrue(vs.source is m)

        m = {"a": "1", "b": 2, "always_ignore_mismatches": True}
        vs = ValueSource(m)
        self.assertTrue(vs.always_ignore_mismatches)
        self.assertTrue(vs.source is m)

    def test_get_values(self):
        m = {"a": "1", "b": 2, "c": {"d": "x", "e": "y"}, "d": {"d": "X"}}
        vs = ValueSource(m)
