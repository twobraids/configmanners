...
some fundemental differences with the way that configmanners and argparse set up
their respective priorities.

One of the primary problems is that both configmanners and argparse have their own
data definition specs.  configmanners has Options while argparse has Actions.  Both
libraries can use their own specs, so a translation layer had to be created.

During the process of configmanners's overlay/expansion phase, the definitions as
...
because the feature sets of argparse and configmanners do not correspond pefectly.
...
configmanners needs to get command line parameters during the overlay/expansion
...
class IntermediateconfigmannersParser(argparse.ArgumentParser):
    ...
    def error(self, message):
        ...
        if (
            ...
            # doing nothing here
            super(IntermediateconfigmannersParser, self).error(message)
...
# ==============================================================================
class FinalStageconfigmannersParser(IntermediateconfigmannersParser):
    # --------------------------------------------------------------------------
    def __init__(self, *args, **kwargs):
        self.get_parser_id()
...
# ==============================================================================
class HelplessconfigmannersParser(IntermediateconfigmannersParser):
    # --------------------------------------------------------------------------
    def __init__(self, *args, **kwargs):
...
# ==============================================================================
class IntermediateconfigmannersSubParser(IntermediateconfigmannersParser):
    # --------------------------------------------------------------------------
    def __init__(self, *args, **kwargs):
        self.get_parser_id()
...
# ==============================================================================
class HelplessIntermediateconfigmannersSubParser(IntermediateconfigmannersParser):
    # --------------------------------------------------------------------------
    def __init__(self, *args, **kwargs):
        new_kwargs = copy.copy(kwargs)
        new_kwargs['add_help'] = False
...
# ==============================================================================
class configmannersAdminParser(IntermediateconfigmannersParser):
    # --------------------------------------------------------------------------
    def __init__(self, *args, **kwargs):
        self.get_parser_id()
        new_kwargs = copy.copy(kwargs)
...
# ==============================================================================
class HelplessconfigmannersAdminParser(IntermediateconfigmannersParser):
    # --------------------------------------------------------------------------
    def __init__(self, *args, **kwargs):
        self.get_parser_id()
        new_kwargs = copy.copy(kwargs)
...
class ParserContainer(object):
    """this class is a argparse ArgumentParser generator.  configmanners uses
    it to create sets of linked ArgumentParsers of the appropriate types.  It
    does the translations between configmanners and argparse.  First it tries to
    ...
    def _add_argument_from_original_source(self, qualified_name, option):
        ...
        if argparse_foreign_data.flags.subcommand:
            ...
            self.subcommand_configmanners_option = option
    ...
    # --------------------------------------------------------------------------
    def _add_argument_from_configmanners_option(self, qualified_name, option):
