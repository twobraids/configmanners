...
can_handle = (bytes, str, yaml)

file_name_extension = "yml"
...
class ValueSource(object):
    ...
    def __init__(self, source, the_config_manager=None):
        ...
        if source is yaml:
            try:
                ...
                source = f"{app.value.app_name}.{file_name_extension}"
        ...
        if isinstance(source, str) and source.endswith(file_name_extension):
            try:
                ...
                raise LoadingJsonFileFailsException(f"Cannot load json: {x}")
        else:
    ...
    def write(source_dict, output_stream=sys.stdout):
        yaml_dict = ValueSource.namespace_to_value_dict(source_dict)
        yaml.dump(yaml_dict, output_stream, default_flow_style=False)
